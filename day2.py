...
s2 = '''Game 1: 1 blue, 8 green; 14 green, 15 blue; 3 green, 9 blue; 8 green, 8 blue, 1 red; 1 red, 9 green, 10 blue
...
def getAns(sample):
    ...
    gameRegex = re.compile(r'\s(\d+)\s(red|blue|green)')
    gamesDontWork = []
    allGames = []
    for i in sample:
        cLoc = i.find(":")
        gameNum = int(i[5:cLoc])
        allGames.append(gameNum)
        mo = gameRegex.findall(i)
        for m in mo:
            color = m[1]
            num = int(m[0])
            if color=='red' and num>12:
                gamesDontWork.append(gameNum)
            elif color=='green' and num>13:
                gamesDontWork.append(gameNum)
            elif color=='blue' and num>14:
                gamesDontWork.append(gameNum)
    gamesDontWork = set(gamesDontWork)

    gamesDoWork = [x for x in allGames if x not in gamesDontWork]
    answer = sum(gamesDoWork)
    return answer
print(getAns(sample))
print(getAns(s2))
