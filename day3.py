...
s2 = '''.......358..........31.....339.....669.............598......328.....575......................447..650..............964...........692........
...
def getAns(sample):
    ...
    notNumOrPeriodRegex = re.compile(r'[^\d\.]')


    js = []
    for n,i in enumerate(sample):
        numbers = numRegex.findall(i)
        iter = re.finditer(r"(\d+)", i)
        indices = [m.start(0) for m in iter]
        for q,j in enumerate(numbers):
            startI = indices[q]
            match=(re.search(j, i[startI:]))
            
            sp = match.span()
            n1 = max(0, sp[0]-1+startI)
            n2 = min(len(i)-1, sp[1]+1+startI)
            crowSpan = i[n1:n2]
            if len(notNumOrPeriodRegex.findall(crowSpan))>0:
                js.append(int(j))
            else:
                if n==0:
                    nr = sample[n+1]
                    span = nr[n1:n2]
                    if len(notNumOrPeriodRegex.findall(span))>0:
                        js.append(int(j))
                elif n==len(sample)-1:
                    nr = sample[n-1]
                    span = nr[n1:n2]
...
def getAns2(sample):
    ...
    for n,i in enumerate(sample):
        ...
        for q,j in enumerate(numbers):
            ...
            if len(starRegex.findall(crowSpan))>0:
                iterS = re.finditer(r"\*", crowSpan)
                indicesS = [m.start(0) for m in iterS]
                for s in indicesS:
                    starPos = str(str(n)+"_"+str(s+n1))
                    try:
                        existingEntry = starsAndNums[starPos]
                        if type(existingEntry)==list:
                            starsAndNums[starPos] = existingEntry.append(j)
                        else:
                            nl = []
                            nl.append(existingEntry)
                            nl.append(j)
                            starsAndNums[starPos] = nl
                    except:                   
                        starsAndNums[starPos] = j
            else:
                if n==0:
                    nr = sample[n+1]
                    span = nr[n1:n2]
                    if len(starRegex.findall(span))>0:
                        ###
                        iterS = re.finditer(r"\*", span)
                        indicesS = [m.start(0) for m in iterS]
                        for s in indicesS:
                            starPos = str(str(n+1)+"_"+str(s+n1))
                            try:
                                existingEntry = starsAndNums[starPos]
                                if type(existingEntry)==list:
                                    starsAndNums[starPos] = existingEntry.append(j)
    ...
    for k in starsAndNums.keys():
        entry = starsAndNums[k]
        if len(entry)==2 and type(entry)==list:
            power = int(entry[0])*int(entry[1])
            rsum += power
